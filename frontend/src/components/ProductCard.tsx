...
//   onAddToCart?: () => void;
//   viewMode?: "grid" | "list";
//   isVisible?: boolean; // Prop để điều khiển animation dựa trên IntersectionObserver
//   animationDelay?: string; // Prop cho animation delay
// }
...
//   // Common animation properties for both grid and list view cards
...
//               {/* Content Section - Adjusted to be a flex column */}
//               <div className="flex flex-col justify-between flex-1 space-y-3">
//                 <div>
//                   {" "}
//                   {/* This div wraps content that should stick to the top */}
...
//                 {/* Price and Actions - Pushed to bottom by justify-between on parent */}
...
//           {" "}
//           {/* Footer that pushes to the bottom */}
...
import {
  ...
  BookOpen,
  Heart,
  HeartCrack, // Icon khi xóa khỏi yêu thích (tùy chọn)
} from "lucide-react";
...
interface ProductCardProps {
  ...
  onRemoveFromWishlist?: (productId: string, productName: string) => void; // ✅ Prop cho hàm xóa từ Wishlist.tsx
}
...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  isVisible = true,
  animationDelay = "0ms",
  onRemoveFromWishlist, // Destructure the new prop
}) => {
  const { addToCart } = useCart();
  const { wishlist, addToWishlist, removeFromWishlist, isInWishlist } =
    useWishlist(); // ✅ Sử dụng hook wishlist
  ...
  const handleToggleWishlist = (e: React.MouseEvent) => {
    ...
    e.stopPropagation(); // Ngăn chặn sự kiện lan truyền lên thẻ cha

    if (isInWishlist(product.id)) {
      removeFromWishlist(product.id);
      // Nếu nút này được gọi từ trang Wishlist, gọi prop onRemoveFromWishlist để cập nhật UI cha
