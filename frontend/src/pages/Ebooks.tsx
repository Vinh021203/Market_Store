...
const Ebooks: React.FC = () => {
  ...
  const priceRanges = [
    ...
    { label: "Trên 200K", value: [200000, Infinity] as [number, number] },
  ];
    ...
    <div className="container px-4 py-8 mx-auto">
      ...
      <div className="mb-8 space-y-4">
        ...
        <p className="text-lg text-muted-foreground">
          Khám phá bộ sưu tập e-books chất lượng cao về lập trình và thiết kế
        </p>
      </div>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredProducts.map((product) => (
              <ProductCard key={product.id} product={product} showAddToCart onAddToCart={() => addToCart(product)}/>
            ))}
          </div>
        ) : (
