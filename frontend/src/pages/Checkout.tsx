...
const Checkout: React.FC = () => {
  ...
  const onSubmit = async (data: CheckoutData) => {
    ...
    try {
      // ✅ SỬA - Chỉ gửi các field tồn tại trong database như mã 2
      const orderData = {
        user_id: user.id,
        full_name: data.name,
        email: data.email,
        ...
        total_price: finalPrice, // ✅ Sử dụng finalPrice (đã trừ discount)
        status: "pending",
        payment_status: "pending",
        // ✅ LƯU DISCOUNT VÀO NOTES FIELD (nếu có)
        ...(appliedDiscount && {
          notes: `Mã giảm giá: ${appliedDiscount.code} - Tiết kiệm: ${formatPrice(discountAmount)} - Giá gốc: ${formatPrice(totalPrice)}`,
        }),
      };
      ...
      // ✅ XÓA PHẦN NÀY VÌ KHÔNG CẦN THIẾT (database không có bảng discount_usage)
      // if (appliedDiscount) {
