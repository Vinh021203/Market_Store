...
export const getAnalyticsData = async (timeRange: string): Promise<AnalyticsData> => {
  try {
    ...
    const prevRevenue = prevRevenueData?.reduce((sum, order) => sum + (order.total_price || 0), 0) || 0;

    // Calculate growth rates
    const revenueGrowth = prevRevenue > 0 ? ((totalRevenue - prevRevenue) / prevRevenue * 100) : 0;
    const ordersGrowth = prevOrders ? (((totalOrders || 0) - prevOrders) / prevOrders * 100) : 0;
    const customersGrowth = prevCustomers ? (((newCustomers || 0) - prevCustomers) / prevCustomers * 100) : 0;
    const downloadsGrowth = prevDownloads ? (((totalDownloads || 0) - prevDownloads) / prevDownloads * 100) : 0;
...
const getDaysArray = (timeRange: string) => {
  ...
  if (timeRange === "30d") daysCount = 7; // Show last 7 days for 30d range
...
export const exportAnalyticsReport = async (timeRange: string) => {
  try {
    ...
    const kpiData = [
      ...
      ['Khách hàng mới', data.kpiData.newCustomers, data.kpiData.customersGrowth],
      ['Tổng lượt tải', data.kpiData.totalDownloads, data.kpiData.downloadsGrowth],
    ];
    ...
    if (data.revenueData.length > 0) {
      const revenueHeaders = ['Tháng', 'Doanh thu', 'Đơn hàng', 'Khách hàng mới'];
      const revenueData = [
        revenueHeaders,
      ...
      XLSX.utils.book_append_sheet(workbook, revenueWorksheet, 'Doanh thu theo tháng');
    }
    
    // Sheet 3: Sản phẩm bán chạy
    if (data.topProducts.length > 0) {
      const productsHeaders = ['Thứ hạng', 'Tên sản phẩm', 'Số lượng bán', 'Doanh thu'];
      const productsData = [
        productsHeaders,
      ...
      XLSX.utils.book_append_sheet(workbook, productsWorksheet, 'Sản phẩm bán chạy');
    }
    ...
    XLSX.utils.book_append_sheet(workbook, categoryWorksheet, 'Phân bố danh mục');
    ...
    if (data.dailyVisitors.length > 0) {
      ...
      XLSX.utils.book_append_sheet(workbook, visitorsWorksheet, 'Lượt truy cập');
    }
...
export const exportStyledAnalyticsReport = async (timeRange: string) => {
  try {
    ...
    const kpiData = [
      ['Chỉ số', 'Giá trị', 'Tăng trưởng (%)'],
      ['Tổng doanh thu', data.kpiData.totalRevenue.toLocaleString('vi-VN'), `${data.kpiData.revenueGrowth.toFixed(1)}%`],
      ['Tổng đơn hàng', data.kpiData.totalOrders.toLocaleString('vi-VN'), `${data.kpiData.ordersGrowth.toFixed(1)}%`],
      ['Khách hàng mới', data.kpiData.newCustomers.toLocaleString('vi-VN'), `${data.kpiData.customersGrowth.toFixed(1)}%`],
      ['Tổng lượt tải', data.kpiData.totalDownloads.toLocaleString('vi-VN'), `${data.kpiData.downloadsGrowth.toFixed(1)}%`],
    ];
    
    const kpiWorksheet = createStyledWorksheet(kpiData, 'BÁO CÁO TỔNG QUAN KPI');
    kpiWorksheet['!cols'] = [
      { wch: 25 },
      { wch: 20 },
      { wch: 18 },
